...
(function() {
	...
	function init(item) {
		...
		var slide_counter = document.createElement('div');
		slide_counter.className = 'slide_counter';
		slide_counter.innerHTML = "<span class='slide_counter_big'>1</span><span class='slide_counter_small'>/"+items.length+"</span>";

		if (items.length > 1) {
			slide_nav.appendChild(prevbtn);
			slide_nav.appendChild(nextbtn);
            item.appendChild(slide_nav);
            item.appendChild(slide_counter);
		}
		...
		var navigate = function(dir) {
			...
			slide_counter.firstChild.textContent = current + 1;
		}
